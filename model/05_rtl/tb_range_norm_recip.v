...
module tb_range_norm_recip;
    ...
    range_norm #(.DIM(DIM), .N_CU(N_CU), .GAMMA_SHIFT(-1),
        .GAMMA_FILE("gamma_test.hex"), .BETA_FILE("beta_test.hex")) u_div (
    ...
    range_norm_recip #(.GAMMA_FILE("gamma_test.hex"), .BETA_FILE("beta_test.hex")) u_rec (
        .clk(clk), .rst_n(rst_n), .block_sel(1'b0), .gamma_shift_in(gamma_shift_in),
